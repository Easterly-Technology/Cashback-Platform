...
export default async function MerchantDashboard() {
  ...
  const quickActions = [
    {
      ...
      description: "Build a customer cart and generate a live QR in a few taps.",
      icon: QrIcon,
      tone: "hero",
    },
    {
  ...
  const insightLines = [
    openQrCount > 0
      ? `${openQrCount} checkout QR ${openQrCount === 1 ? "is" : "are"} still open right now.`
          ...
          {recentTxns.length === 0 ? (
            <div className="material-empty border-emerald-200 bg-emerald-50/40 px-6 py-10 text-center">
              ...
              <p className="mt-2 text-sm text-slate-500">
                Launch a QR checkout to start collecting customer confirmations and settlement data.
              </p>
            </div>
          ) : (
            <>
