...
import {
  ...
  marketplaceBuySchema as tradeOrderSchema,
  marketplaceIdempotencyKeySchema as tradeIdempotencyKeySchema,
} from "@cashback/shared";
import { getTradeSnapshotUncached } from "@/features/trade/server/trade-snapshot";
...
export async function GET() {
  ...
  return Response.json(await getTradeSnapshotUncached(userId));
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsedIdempotencyKey = idempotencyKeyHeader
      ? tradeIdempotencyKeySchema.safeParse(idempotencyKeyHeader)
      : null;
    ...
    if (!getTokenSettingsSnapshot(settings).marketplaceEnabled) {
      return Response.json(
        { error: "Trade is currently disabled" },
        { status: 409 },
      );
    }
    ...
    console.error("[USER] Trade failed:", error);
