...
async function ReleaseForecast({ userId }: { userId: string }) {
  const [entitlement, settings] = await Promise.all([
    prisma.userTokenEntitlement.findUnique({ where: { userId } }),
    prisma.platformSetting.findMany({
      where: { key: { in: ["token_release_rate"] } },
  ...
  return (
    <ReleaseForecastCard
      entitledTokens={Number(entitlement?.entitledTokens ?? 0)}
      releasedTokens={Number(entitlement?.releasedTokens ?? 0)}
      availableTokens={Number(entitlement?.availableTokens ?? 0)}
